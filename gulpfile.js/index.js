...
function babelAndCopyFile() {
  return gulp.src(envOptions.javascript.src)
  .pipe($.sourcemaps.init())
  .pipe($.babel({
    presets: ['@babel/env'],
  }))
  .pipe(gulp.src(envOptions.javascript.src))
  .pipe(gulp.dest(envOptions.javascript.path))
  .pipe(
    browserSync.reload({
      stream: true,
...
function watch() {
  ...
  gulp.watch(envOptions.javascript.src, gulp.series(babelAndCopyFile));
...
exports.build = gulp.series(clean, copyFile, layoutHTML, sass, babelAndCopyFile, vendorsJs);

exports.default = gulp.series(clean, copyFile, layoutHTML, sass, babelAndCopyFile, vendorsJs, gulp.parallel(browser, watch));
